...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Ella",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Ella");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Ella", id);
      await deleteDoc(a);
