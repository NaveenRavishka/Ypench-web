...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Nilaweli",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Nilaweli");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Nilaweli", id);
      await deleteDoc(a);
