...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Hikkaduwa",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Hikkaduwa");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Hikkaduwa", id);
      await deleteDoc(a);
