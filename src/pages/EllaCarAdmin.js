...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "EllaCar",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "EllaCar");
const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const productDoc = doc(db, "EllaCar", id);
        ...
        <TableBody>
          {user.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              handleDelete(row.id);}} >Delete</Button> </StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
