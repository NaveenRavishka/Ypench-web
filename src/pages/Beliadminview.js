...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Belihuloya",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Belihuloya");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Belihuloya", id);
      await deleteDoc(a);
