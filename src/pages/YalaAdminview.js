...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Yala",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Yala");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Yala", id);
      await deleteDoc(a);
