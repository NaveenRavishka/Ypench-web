...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Sigiriya",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Sigiriya");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Sigiriya", id);
      await deleteDoc(a);
...
export default SigiriAdminview