...
// ];
 ...
 useEffect( ()=> {
    setLoading(true);
    const unsub = onSnapshot(collection(db, "Adam'sPeek",), (snapshot)=> {
            let list =[];
...
const productsCollectionRef = collection(db, "Adam'sPeek");

const handleDelete= async (id)=>{
  if (window.confirm("Are you sure you want to delete")){
    try{
      const a = doc(db, "Adam'sPeek", id);
      await deleteDoc(a);
